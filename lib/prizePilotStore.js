...
function hashRecoveryPassphrase(passphrase) {
  const normalized = String(passphrase || "").trim();
  if (normalized.length < 8) {
    throw new StoreError("Recovery passphrase must be at least 8 characters.", 400);
  }
  const salt = randomBytes(16).toString("hex");
  const hash = scryptSync(normalized, salt, 64).toString("hex");
  return `${salt}:${hash}`;
...
function verifyRecoveryPassphrase(passphrase, storedHash) {
  if (!storedHash || !storedHash.includes(":")) {
    return false;
  }
  const [salt, expectedHex] = storedHash.split(":");
  const expected = Buffer.from(expectedHex, "hex");
  const actual = scryptSync(String(passphrase || "").trim(), salt, expected.length);
  return timingSafeEqual(actual, expected);
}
...
function validateRegisterInput(input) {
  ...
  const recoveryPassphrase = String(input.recoveryPassphrase || "");
  ...
  return { username, organizerName, businessName, email, password, recoveryPassphrase };
}
...
function validateRecoveryResetInput(input) {
  const username = normalizeUsername(input.username);
  const recoveryPassphrase = String(input.recoveryPassphrase || "");
  const password = String(input.password || "");
  if (!USERNAME_PATTERN.test(username)) {
  ...
  if (!recoveryPassphrase.trim()) {
    throw new StoreError("Recovery passphrase is required.", 400);
  }
  return { username, recoveryPassphrase, password };
}
...
function validateEntrantInput(input) {
  ...
  const userAgentHash = normalizeText(input.userAgentHash || "", 160);
  ...
  return { name, email, source, ipHash, userAgentHash, submissionTitle, projectLink, submissionImageData };
}
...
async function ensureSqliteStore() {
  ...
  try {
    db.exec("ALTER TABLE users ADD COLUMN recovery_passphrase_hash TEXT NOT NULL DEFAULT ''");
  } catch {}
  try {
  ...
  try {
    db.exec("ALTER TABLE entrants ADD COLUMN user_agent_hash TEXT NOT NULL DEFAULT ''");
  } catch {}
  try {
  ...
  try {
    db.exec("CREATE INDEX IF NOT EXISTS idx_entrants_campaign_ua_hash ON entrants (campaign_id, user_agent_hash)");
  } catch {}
  try {
...
async function ensurePostgresStore() {
  ...
  await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS recovery_passphrase_hash TEXT NOT NULL DEFAULT ''`);
  await pool.query(
  ...
  await pool.query(
    `
      UPDATE campaigns
      SET workspace_owner_id = COALESCE(NULLIF(user_id, ''), '')
  ...
  await pool.query(`ALTER TABLE entrants ADD COLUMN IF NOT EXISTS user_agent_hash TEXT NOT NULL DEFAULT ''`);
  await pool.query(
  ...
  await pool.query(
    `CREATE INDEX IF NOT EXISTS idx_entrants_campaign_ua_hash ON entrants (campaign_id, user_agent_hash)`
  );
  await pool.query(
    `CREATE UNIQUE INDEX IF NOT EXISTS idx_judging_unique_vote ON judging_scores (campaign_id, entrant_id, judge_user_id)`
  );
...
async function settleExpiredCampaignsForWorkspaceSqlite(workspaceOwnerId) {
  const db = await getSqliteDb();
  const rows = db
    .prepare(
      `
  ...
  const processedIds = [];
  for (const row of rows) {
    const updated = await closeCampaignAndPickWinnerSqlite(row.id, workspaceOwnerId);
    if (updated?.id) {
      processedIds.push(updated.id);
    }
  }
...
async function settleExpiredCampaignsForWorkspacePostgres(workspaceOwnerId) {
  const pool = getPgPool();
  const result = await pool.query(
    `
      SELECT id
      ...
      WHERE workspace_owner_id = $1
        AND status = 'live'
        AND ends_at <> ''
        AND ends_at::timestamptz <= NOW()
    `,
    [workspaceOwnerId]
  );
  const processedIds = [];
  for (const row of result.rows) {
    const updated = await closeCampaignAndPickWinnerPostgres(row.id, workspaceOwnerId);
    if (updated?.id) {
      processedIds.push(updated.id);
    }
  }
...
export async function registerOrganizer(input) {
  ...
  const recoveryPassphraseHash = hashRecoveryPassphrase(validated.recoveryPassphrase);
  ...
  if (backend === "postgres") {
    ...
    try {
      await pool.query(
        `
          INSERT INTO users (id, username, password_hash, recovery_passphrase_hash, organizer_name, business_name, email, workspace_owner_id)
...
export async function resetPasswordWithRecoveryPassphrase(input) {
  const backend = await ensureStore();
  const validated = validateRecoveryResetInput(input);
  const passwordHash = hashPassword(validated.password);
  ...
  if (backend === "postgres") {
    ...
    const userResult = await pool.query(
      `
        SELECT id, recovery_passphrase_hash
        FROM users
        WHERE username = $1
        LIMIT 1
      `,
    ...
    const recoveryHash = user?.recovery_passphrase_hash || "";
    if (!user || !recoveryHash) {
      throw new StoreError("Recovery passphrase reset is not enabled for this account yet.", 400);
    }
    if (!verifyRecoveryPassphrase(validated.recoveryPassphrase, recoveryHash)) {
      throw new StoreError("Invalid recovery passphrase.", 401);
    }
...
export async function submitCampaignEntry(campaignId, input) {
  ...
  const userAgentHash = normalizeText(entry.userAgentHash, 160);
  ...
  if (backend === "postgres") {
    ...
    if (campaign.trust_mode === "high_trust" && (ipHash || userAgentHash)) {
      const ipCheck = await pool.query(
        `
          ...
          WHERE campaign_id = $1
            AND (
              ($2 <> '' AND ip_hash = $2)
              OR ($3 <> '' AND user_agent_hash = $3)
            )
          LIMIT 1
        `,
        [normalizedId, ipHash, userAgentHash]
      );
      if (ipCheck.rows[0]) {
    ...
    const recentAbuseCheck = await pool.query(
      `
        SELECT COUNT(*)::int AS count
        FROM entrants
        WHERE email = $1
    ...
    if (Number(recentAbuseCheck.rows[0]?.count || 0) >= 12) {
      throw new StoreError(
        "Entry rate limit reached for this email. Please try again later.",
        429
      );
    }
    ...
    await pool.query(
      `
        INSERT INTO entrants (id, campaign_id, user_id, name, email, source, ip_hash, user_agent_hash, submission_title, submission_image_data, submission_link)
    ...
    const ipDuplicate = db
      .prepare(
        ...
        WHERE campaign_id = ?
          AND (
            (? != '' AND ip_hash = ?)
            OR (? != '' AND user_agent_hash = ?)
          )
        LIMIT 1
      `
      )
      .get(normalizedId, ipHash, ipHash, userAgentHash, userAgentHash);
    if (ipDuplicate) {
      throw new StoreError("This device has already submitted an entry for this campaign.", 409);
    }
  }
  ...
  const recentAbuseCount = db
    ...
    .get(entry.email);
  if (Number(recentAbuseCount?.count || 0) >= 12) {
    throw new StoreError("Entry rate limit reached for this email. Please try again later.", 429);
  }
...
export async function settleExpiredCampaignsForSession(sessionToken) {
  const access = await getWorkspaceAccess(sessionToken);
  if (!access.isOwner) {
    throw new StoreError("Only workspace owners can run settle actions.", 403);
  }

  const backend = await ensureStore();
  const summary =
    backend === "postgres"
      ? await settleExpiredCampaignsForWorkspacePostgres(access.workspaceOwnerId)
      : await settleExpiredCampaignsForWorkspaceSqlite(access.workspaceOwnerId);
